...
const toneText: Record<string, string> = {
  saffron: "from-black/60 to-black/90",
  maroon: "from-black/70 to-black/95",
  gold: "from-black/50 to-black/80",
  krishna: "from-black/60 to-black/90",
};

/** Asymmetric bento of temple experiences. */
export function Explore() {
  return (
    <section id="explore" className="relative bg-[var(--color-bg-primary)] bg-dots py-20 sm:py-28">
        ...
        <div className="mt-14 grid auto-rows-[15rem] grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {EXPERIENCES.map((exp, i) => {
            ...
            return (
              ...
              >
                <article
                  className={`group relative h-full overflow-hidden rounded-3xl shadow-luxury ${
                    tall ? "min-h-full" : ""
                  }`}
                >
                  ...
                  <div className="relative flex h-full flex-col justify-end p-6">
                    <span className="mb-3 grid h-12 w-12 place-items-center rounded-2xl border border-white/25 bg-white/15 text-white backdrop-blur">
                      <Icon name={exp.icon} className="h-6 w-6" />
                    </span>
                    <h3 className="font-display text-2xl text-white">{exp.title}</h3>
                    <p className="mt-1.5 max-w-xs text-sm leading-relaxed text-white/80">
                      {exp.blurb}
                    </p>
                    <span className="mt-4 inline-flex items-center gap-1 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--color-accent-primary)] opacity-0 transition-opacity duration-300 group-hover:opacity-100">
